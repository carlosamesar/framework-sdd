...
| `AWS_LAMBDA_DEPLOY_ROLE_ARN` | Rol OIDC para despliegue GitHub Actions → AWS | Recomendado para evitar access keys persistentes. |
| `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` | Fallback para despliegue ZIP a Lambda | Solo si OIDC no está disponible; guardar como GitHub Secrets/Environment Secrets. |
| `AWS_REGION` | Región del deploy Lambda | Recomendado fijarla explícitamente aunque el workflow tenga default. |
...
> Para el caso específico de GitHub Actions + Lambda ZIP, los valores del `.env` local deben reflejarse manualmente en GitHub Secrets; ver `docs/GITHUB-ACTIONS-AWS-LAMBDA-DEPLOY.md`.
